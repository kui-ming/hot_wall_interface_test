...
回复
id、评论id、用户id、回复id、内容、创建时间、更新时间、状态
...
- 评论
    ...
    - [X] 登录用户查询自己的评论（可分页和关键字查询）
    - [X] 通过评论用户Id查询评论
    - [X] 通过文章Id查询评论
    - 逻辑
        - [X] 所有查询评论信息增加评论用户名、用户头像、文章名、文章作者、文章简介、回复数量
- 回复
    - [X] 添加
        - [X] 父评论Id必须是有效的
        - [X] 回复他人回复时他人回复Id必须是有效的
        - [X] 登录用户无需传入user_id
    - [X] 无法修改
    - [X] 删除
        - [X] 只能删除自己的回复
    - [X] 登录用户查询自己的回复(分页形式)
    - [X] 通过父评论id查询相关回复（分页形式）
    - 逻辑
        - [X] 所有查询回复信息增加用户名、用户头像、回复用户名
...
- 喜欢(点赞)
    ...
    - [X] 通过文章id查询点赞用户列表
    - [X] 通过用户id查询点赞文章列表
        - [X] 登录用户可以直接查询自己点赞的文章列表
    - [X] 通过文章id查询点赞总数
    - [X] 通过用户id查询点赞文章总数
        - [X] 登录用户可以直接查询自己点赞的文章总数
    - 逻辑
...
#### 反思
因该把前端用户接口和后端管理接口进行分离的，现在杂在一起很难搞